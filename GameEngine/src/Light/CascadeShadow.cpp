...
vector<glm::vec4> CascadeShadow::GetFrustumCornerWorldSpace(
...
glm::mat4 CascadeShadow::GetDirectionalLightMatrixTemp(
	const float near, 
	const float far,
	const Light light) {
	...
	const auto lightView = glm::lookAt(
		center - (glm::normalize(light.direction) * 100.0f) ,
		center,
...
std::vector<glm::mat4> CascadeShadow::GetLightSpaceMatrices(Light light) {
	...
	for (int i = 0; i < cascadeLevels.size(); i++) {
		if (i == 0) {
			...
			matrices.push_back(GetDirectionalLightMatrix(cascadeLevels[i],cascadeLevels[i+1], light));
		}
	}
